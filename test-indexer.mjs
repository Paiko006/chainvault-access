...
async function run() {
  ...
  console.log("Total Blobs Returned by Indexer:", blobs.length);
  const uniqueNames = new Set(blobs.map(b => b.blob_name));
  console.log("Unique Blob Names:", uniqueNames.size);
  const uniqueTxs = new Set(blobs.map(b => b.transaction_hash));
  console.log("Unique Txs:", uniqueTxs.size);
  console.log("All Blob Names:", blobs.map(b => b.blob_name).sort());
}
run();
