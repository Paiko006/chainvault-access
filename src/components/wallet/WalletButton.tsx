    ...
    <div className="relative">
      ...
      >
        <div className="h-5 w-5 shrink-0">
          {(wallet as { icon?: string; name?: string })?.icon ? (
            <img src={(wallet as { icon?: string; name?: string }).icon} alt={(wallet as { icon?: string; name?: string }).name} className="h-full w-full object-contain" />
          ) : (
        ...
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 w-56 rounded-xl border border-border bg-card shadow-xl p-2 space-y-1">
            <div className="px-3 py-2 border-b border-border/50 mb-1">
              <div className="text-xs text-muted-foreground">{(wallet as { name?: string })?.name}</div>
              <div className="text-xs font-mono text-foreground mt-0.5">{shortenAddress(account.address.toString())}</div>
            </div>
